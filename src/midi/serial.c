...
{
    *(volatile u8*)regs[io_port].sctrl = value;
}

static void set_ctrl(u8 value)
{
    *(volatile u8*)regs[io_port].ctrl = value;
}

u8 serial_sctrl(void)
{
    return *(volatile u8*)regs[io_port].sctrl;
}

bool serial_readyToReceive(void)
{
    return *(volatile u8*)regs[io_port].sctrl & SCTRL_RRDY;
}

u8 serial_receive(void)
{
    return *(volatile u8*)regs[io_port].rx;
}
...
{
    *(volatile u8*)regs[io_port].tx = data;
}

bool serial_readyToSend(void)
{
    return !(*(volatile u8*)regs[io_port].sctrl & SCTRL_TFUL);
}
